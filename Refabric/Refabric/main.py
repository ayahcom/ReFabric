...
# def findMatchesinCosmetics(word):
#     matches = []
#     if not word:
#         return matches
#     word = word.lower()
#     for element in cosmetics:
#         if (word in element["name"].lower() or
#             word in element["subCategory"].lower() or
#             word in element["category"].lower()):
#             matches.append(element)
#     return matches


# @app.route('/displayResultsCosmetics/')
# def getFormData():
#     search = request.args.get('search')
#     return render_template('displayResultsCosmetics.html', output=findMatchesinCosmetics(search))

# #in html for displaying results do a ul tag and then inside use jinja for "for element in output" and then make it do an li so for each things in matches it adds it to the list as li and then style it or whatever yadayadayada
